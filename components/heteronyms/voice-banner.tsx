...
export function VoiceBanner({ guestHeteronym, onClear }: VoiceBannerProps) {
  ...
  return (
    <div
      className="flex items-center gap-2 px-3 py-1.5 bg-brass-500/10 border-b border-brass-500/20"
      role="status"
    ...
    >
      ...
      <span className="text-xs text-brass-700 flex-1 truncate">
        {t.rich('writingAs', { name: guestHeteronym.name, b: (chunks) => <strong className="font-medium">{chunks}</strong> })}
      </span>
      <button
        onClick={onClear}
        className="p-0.5 text-brass-600 hover:text-brass-800 rounded transition-colors"
