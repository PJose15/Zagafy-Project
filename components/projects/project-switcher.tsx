...
export function ProjectSwitcher({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const activeTitle = active?.title || 'Untitled Project';
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={`Active project: ${activeTitle}. Switch project`}
      >
      ...
      {open && (
        ...
        >
          <p className="px-3 py-1 text-[10px] font-mono uppercase tracking-widest text-brass-400/50">Projects</p>
          {projects.map(p => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 min-w-0 truncate">{p.title || 'Untitled Project'}</span>
                <span className="text-[10px] font-mono text-cream-400/40 shrink-0">{p.chapterCount}ch</span>
              </button>
            );
          })}
